 ...
 * Generated: 2025-10-28T00:06:03.794094
 */
...
class APISequence {
    ...
    substituteVariables(value, stepTimestamp = null) {
        if (typeof value === 'string') {
            ...
            // 2. Global variables - {{ .vars.key }}            for (const [varName, varValue] of Object.entries(this.variables)) {
                const pattern = new RegExp(`\\{\\{\\s*\\.vars\\.${varName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*\\}\\}`, 'g');
                value = value.replace(pattern, String(varValue));
            }

            // 3. User input - {{ .input.variableName }}            for (const [inputName, inputValue] of Object.entries(this.userInputs)) {
                const pattern = new RegExp(`\\{\\{\\s*\\.input\\.${inputName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*\\}\\}`, 'g');
                value = value.replace(pattern, String(inputValue));
            }

            // 4. Response references - {{ .responses.nodeId.field.subfield }}            const responsePattern = /\{\{\s*\.responses\.([a-zA-Z0-9_-]+)\.(.+?)\s*\}\}/g;
...
describe('SimpleApiTest', () => {
    let apiSequence;

    beforeEach(() => {
        const config = {
    "name": "Simple API Test",
    "description": "Basic GET and POST requests",
    "enableDebug": false,
    "defaults": {
