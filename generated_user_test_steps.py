...
class APIContext:
    ...
    def substitute_variables(self, value: Any, step_timestamp: Optional[int] = None) -> Any:
        ...
        Supports:
        - Dynamic placeholders: {{ $guid }}, {{ $timestamp }}        - Global variables: {{ .vars.key }}        - User input: {{ .input.variableName }}        - Response references: {{ .responses.nodeId.field }}        """
...
def get_api_context(context):
    ...
    if not hasattr(context, 'api_context'):
        ...
        config = {"defaults": {"baseUrl": "https://jsonplaceholder.typicode.com", "headers": {"Content-Type": "application/json"}, "timeout": 30, "validations": [{"httpStatusCode": 200}]}, "description": "Test user registration and login", "enableDebug": true, "name": "User API Test", "nodes": [{"extractFields": [{"jsonPath": "[0].id", "variableName": "firstUserId"}], "id": "get_users", "method": "GET", "name": "Get all users", "url": "/users"}, {"id": "get_user_details", "method": "GET", "name": "Get specific user details", "url": "/users/{{ .vars.firstUserId }}", "validations": [{"httpStatusCode": 200}, {"field": "id", "value": "{{ .vars.firstUserId }}"}]}, {"body": {"body": "Created at {{ $timestamp }}", "title": "Test Post {{ $guid }}", "userId": 1}, "id": "create_post", "method": "POST", "name": "Create a new post", "url": "/posts", "validations": [{"httpStatusCode": 201}, {"field": "title", "operator": "contains", "value": "Test Post"}]}], "variables": {"apiVersion": "v1"}}
